...
// 報酬変更を承認（担当者のみ・CHANGE_PENDING → ASSIGNED）
export async function POST(_req: Request, { params }: Params) {
  ...
  if (subQuest.assigneeId !== member.id) {
    return NextResponse.json({ error: "担当者のみ承認できます" }, { status: 403 });
  }

  if (subQuest.status !== "CHANGE_PENDING") {
    return NextResponse.json({ error: "変更承認待ちのサブクエストのみ承認できます" }, { status: 400 });
  }
  ...
  const updated = await prisma.subQuest.update({
    ...
    data: {
      status: "ASSIGNED",
      pointReward: subQuest.pendingPointReward!,
