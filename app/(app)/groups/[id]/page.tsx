...
function BonusRulesSection({ groupId }: { groupId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/groups/${groupId}/bonus-rules`)
      .then((r) => r.ok ? r.json() : [])
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setAdding(true);
    try {
      const res = await fetch(`/api/groups/${groupId}/bonus-rules`, {
        method: "POST",
  ...
  async function handleDelete(ruleId: string) {
    const res = await fetch(`/api/groups/${groupId}/bonus-rules/${ruleId}`, { method: "DELETE" });
  ...
  return (
    <section className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
      <div>
        <h3 className="font-semibold text-gray-800">ボーナス・ペナルティルール（ADMIN）</h3>
        <p className="text-xs text-gray-400 mt-0.5">
          クエストの作成日〜納期の期間を100%として、達成タイミングに応じてボーナス/ペナルティを設定します。
        </p>
      </div>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            <p className="text-xs text-gray-500 mb-1">
              しきい値（%）
              <span className="ml-1 text-gray-400">例: 80=期間の80%、110=10%遅延</span>
            </p>
            <input
              type="number"
              min={1}
          ...
          <div>
            <p className="text-xs text-gray-500 mb-1">
              ボーナス率（%）
              <span className="ml-1 text-gray-400">正=ボーナス、負=ペナルティ</span>
            </p>
            <input
              type="number"
              step={0.1}
