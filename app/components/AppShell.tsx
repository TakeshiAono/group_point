...
import { OnboardingProvider, type OnboardingStep } from "@/lib/onboarding-context";
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch("/api/me").then((r) => r.ok ? r.json() : null).then((data) => {
      setMe(data);
      setInitialStep(data && !data.onboardingCompleted ? "welcome" : null);
    });
  }, []);

  // initialStep が未確定の間は何も表示しない（チラつき防止）
